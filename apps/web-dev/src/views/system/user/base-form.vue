...
import { createUser, queryUserById, updateUser } from '#/api/system/user';
...
const handleSubmit = async () => {
  try {
    ...
    await (isUpdate.value
      ? updateUser(dataId.value, data as User.Req)
      : createUser(data as User.Req));
    resetInitialized();
    emit('refresh');
    open.value = false;
  } catch (error) {
...
const setupForm = async () => {
  ...
  try {
    // 更新模式，。查询数据
    if (dataId.value) {
      const record = await queryUserById(dataId.value);
