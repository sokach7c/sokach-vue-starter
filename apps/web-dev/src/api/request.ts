 ...
 * 初始化加密器
 */
encrypter.setKeys(rsaPrivateKey, rsaPublicKey);
  ...
  client.addRequestInterceptor({
    fulfilled: (config) => {
      ...
      if (
        ...
        config.headers['encrypt-key'] = encrypter.encrypt(
  ...
  client.addResponseInterceptor<HttpResponse>({
    fulfilled: async (response) => {
      ...
      if (encryptKey) {
        ...
        const base64Str = encrypter.decrypt(encryptKey);
      ...
      const { success, code, data, msg, ...other } = axiosResponseData;
